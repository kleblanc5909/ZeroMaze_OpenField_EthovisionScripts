...
resultsColumns = ["Subject", "Group", "Gender", "Timestamp", "Notes", "Time in open (%)", "Time in open while moving (%)", "Number of movements", "Average duration of movements (s)", "Total time moving (s)", "Speed while moving (cm/s)", "Average velocity (cm/s)", "Number of movements in open"]
myDataList = []
os.chdir(dataFolder)
...
for File in FileList:
    ...
    Dataz = [Subject,Genotype,Gender, Timestamp, Notes, PercentTimeinOpen, PercentTimeinOpenMoving, MovementBlocks, AvgMoveDuration, TotalMoveDuration, AvgVelocityMoving, AvgVelocity, OpenTrue]
...
writer = pd.ExcelWriter('KO_and_WT_Zero_Maze_Movement_Analysis.xlsx')
